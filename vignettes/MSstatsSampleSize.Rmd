...
1. `estimateVar`: estimates the mean and variance (standard deviation) for each protein.
...
1. `meanSDplot`: draw the plot for the mean protein abundance vs sample standard deviation for the input preliminary dataset. It can exhibit the quality of input data.
...
The function fits intensity-based linear model on the prelimiary data (Input, here is `data`). This function outputs mean and variance components for each protein.
...
* `log2Trans` : Logical. If TRUE, the input `data' is log-transformed with base 2. Default is FALSE.
...
# Read in annotation including condition, biological replicates and MS runs.
...
This function draws the plot for the mean protein abundance (X-axis) vs standard deviation (Y-axis). The `lowess` function is used to fit the LOWESS smoother between meann protein abundance and standard deviation (square root of variance). This function generates one pdf file with mean-SD plot.


#### Arguments

* `data` : A list with mean protein abundance vector and standard deviation vector. It should be the output of `estimateVar` function.
...
* `optimal_threshold`: The maximal cutoff for deciding the optimal sample size. Default is 0.0001. Large cutoff can lead to smaller optimal sample size
